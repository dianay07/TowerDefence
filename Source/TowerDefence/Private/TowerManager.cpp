...
void ATowerManager::SpawnTowers()
{
	if (TowerLocations.IsEmpty())
	{
		...
		UGameplayStatics::GetAllActorsOfClass(
			GetWorld(),
			AStaticMeshActor::StaticClass(),   // ã�� Ŭ����
			FoundActors              // ��� ���� �迭
		);
	...
	for (AActor* Actor : TowerLocations)
	{
		...
		// BP_BaseTowerClass �� Ŭ������ BP_BaseTowerClass�� �����ؼ� visible ���� ������ BP_BaseTowerClass�� C++ ������ ó��
		// Ȥ�� BP ���� ó�� �Ѵ�.
	}
...
void ATowerManager::UpdateHightlight()
{
	...
	// Sequence Then 1: NewTower != HighlightedTower üũ
...
{
	Super::BeginPlay();
	if (!HasAuthority()) return; // 클라이언트는 스폰 안 함

	// GameMode에 자기 등록 — 레벨 BP의 수동 "Set TowerManager" 노드 불필요
	if (ATDGameMode* GM = Cast<ATDGameMode>(GetWorld()->GetAuthGameMode()))
	{
		GM->TowerManagerActor = this;
	}
