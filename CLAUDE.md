...
| 2026-04-28 | `UTDLobbySessionSubsystem` 1차 구현 — Create / Find / Join / Destroy + 4 델리게이트, `FTDSessionInfo` USTRUCT 래핑으로 BP 노출, `MultiStageId` 보관 추가 | dianay07 |
| 2026-04-28 | `UTDPlayModeSelectWidget` 도입 — 스테이지 진입 시 Solo / Multi 선택 분기. Solo→`UTDLevelSessionSubsystem`, Multi→`UTDLobbySessionSubsystem::SetMultiStageId` + `WBP_MultiLobby` 표시 | dianay07 |
| 2026-04-29 | Solo 모드 안정화 + 잔여 버그 수정 (`[In Play] Start Solo Mode & Bug Fix`) | dianay07 |
| 2026-04-30 | 로비 UI 전면 C++ 포팅 — `UTDMultiLobbyWidget` / `UTDWaitingRoomWidget` / `UTDSessionEntryWidget` / `UTDPlayerEntryWidget`. 2 인 PIE Multi Play Test 통과. § 11-4 위젯 명명을 실제 구현에 맞춰 갱신 (`UTDLobbyMenuWidget` → 5 위젯 분리) | dianay07 |
...
| 멀티 스테이지 ID 보관 (호스트가 선택한 맵) | `UTDLobbySessionSubsystem::MultiStageId` — `Server_RequestStart` 가 맵 경로 조회에 사용 | 로컬 (호스트 측만 의미) |
| 솔로 모드 진입 (멀티 세션 미경유) | `UTDPlayModeSelectWidget::RequestSoloMode` → `UTDLevelSessionSubsystem::RequestLoadStage` | 로컬 |
...
| 메뉴/룸 UI (모드 선택, 방 목록, 참가자 목록) | `UTDPlayModeSelectWidget` / `UTDMultiLobbyWidget` / `UTDWaitingRoomWidget` + 한 줄 위젯 2 종 | 로컬 |
...
```
[스테이지 선택 → PlayMode 진입]
SelectStage(StageId) → UTDPlayModeSelectWidget::InitWithStage(StageId)
  ├ Solo  → RequestSoloMode  → UTDLevelSessionSubsystem::RequestLoadStage(StageId)   [싱글 분기, 멀티 세션 미경유]
  └ Multi → RequestMultiMode → UTDLobbySessionSubsystem::SetMultiStageId(StageId)
                                  + OnMultiModeRequested(StageId) → BP 가 WBP_MultiLobby 표시
...
ATDLobbyPlayerController::Server_RequestStart  (호스트 PC 만 허용)
   ├ LobbySubsystem::GetMultiStageId() 로 호스트가 선택한 StageId 조회
   ├ DT_Stages 에서 맵 경로 검색 (없으면 GameLevelPath 기본값)
   └ Authority: ServerTravel("<MapPath>?listen", true)
         ↓
...
├── Public/
│   ├── Session/
│   │   ├── TDLobbySessionSubsystem.h       [DONE] OnlineSession 단일 허브 + FTDSessionInfo + MultiStageId
│   │   └── TDLevelSessionSubsystem.h       (기존 — 스테이지 전환 담당, 별개 책임)
│   ├── Lobby/
│   │   ├── TDLobbyGameMode.h               [DONE] PostLogin/Logout → GameState 슬롯 갱신
│   │   ├── TDLobbyGameState.h              [DONE] PlayerSlots (Replicated)
│   │   ├── TDLobbyPlayerController.h       [DONE] Server_RequestStart, Server_SetReady, GameLevelPath
│   │   └── TDLobbyPlayerState.h            [DONE] DisplayName, bIsReady (Replicated)
│   └── UI/
│       ├── TDPlayModeSelectWidget.h        [DONE] Solo/Multi 분기 + 스테이지 ID 보관 (메인 메뉴 진입점)
│       ├── TDMultiLobbyWidget.h            [DONE] 방 목록 브라우저 (Create/Refresh/Back)
│       ├── TDWaitingRoomWidget.h           [DONE] 참가 대기실 (참가자 슬롯 + Ready/Start)
│       ├── TDSessionEntryWidget.h          [DONE] 방 목록 한 줄 (호스트명/인원/Join)
│       └── TDPlayerEntryWidget.h           [DONE] 참가자 한 줄 (DisplayName/Ready)
└── Private/ (대응)

※ 명명 변경: 계획 시점의 `UTDLobbyMenuWidget` / `UTDLobbyRoomWidget` / `UTDLobbyEntryWidget` 은
   책임을 더 잘게 쪼개기 위해 위 5 개 위젯으로 분리 구현됨.
   - 방 브라우저(`UTDMultiLobbyWidget`) 와 대기실(`UTDWaitingRoomWidget`) 분리 → 호스트/클라 흐름 단순화.
   - 한 줄 위젯도 방용(`UTDSessionEntryWidget`) / 참가자용(`UTDPlayerEntryWidget`) 분리 → 데이터 모델 명확화.
   - `UTDPlayModeSelectWidget` 추가 → 솔로(`UTDLevelSessionSubsystem`) 와 멀티(`UTDLobbySessionSubsystem`) 분기를 단일 위젯에서 처리.
...
Content/Levels/Lobby.umap                          [DONE] BP_TDLobbyGameMode 사용
Content/UI/Lobby/WBP_PlayModeSelect.uasset         [DONE] UTDPlayModeSelectWidget 자식
Content/UI/Lobby/WBP_MultiLobby.uasset             [DONE] UTDMultiLobbyWidget 자식
Content/UI/Lobby/WBP_WaitingRoom.uasset            [DONE] UTDWaitingRoomWidget 자식
Content/UI/Lobby/WBP_SessionEntry.uasset           [DONE] UTDSessionEntryWidget 자식
Content/UI/Lobby/WBP_PlayerEntry.uasset            [DONE] UTDPlayerEntryWidget 자식
```

### 11-5. 실행 단계 (작은 PR 단위)

각 단계는 독립 빌드 가능. 1~6 단계는 2026-04-30 기준 모두 완료, 2 인 PIE Multi Play Test 통과.

1. **모듈 / 플러그인 / Config 활성화** ✅ DONE
...
2. **`UTDLobbySessionSubsystem` 도입** ✅ DONE
   ...
   - `FTDSessionInfo` USTRUCT 래핑 → `FOnlineSessionSearchResult` 의 BP 노출 한계 우회.
   - `MultiStageId` 보관 + `SetMultiStageId/GetMultiStageId` 추가 (`Server_RequestStart` 가 사용).
3. **로비 레벨 + GM/GS/PC/PS** ✅ DONE
   ...
   - `ATDLobbyPlayerController::Server_SetReady`, `Server_RequestStart` (Authority 게이트), `GameLevelPath` 기본값.
4. **메뉴 / 룸 위젯** ✅ DONE
   - 5 위젯 C++ 베이스 + BP 자식 분리 — `UTDPlayModeSelectWidget` / `UTDMultiLobbyWidget` / `UTDWaitingRoomWidget` / `UTDSessionEntryWidget` / `UTDPlayerEntryWidget`.
   - § 11-4 의 명명 변경 사유 참고 (책임 더 잘게 분할).
   - 패턴은 § 3-1 의 `UTDTowerActionWidgetBase` 와 동일: 데이터/로직 C++, 시각 BP.
5. **게임 시작 트래블** ✅ DONE
   - `Server_RequestStart` 내부에서 `GetWorld()->ServerTravel("<MapPath>?listen", true)`.
   - 맵 경로는 `LobbySubsystem::GetMultiStageId()` → `DT_Stages` 조회. 미설정 시 `GameLevelPath` 기본값.
...
- **PIE 멀티 (Listen Server)** — 2026-04-30 2 인 Multi Play Test 통과 ✅
  ...
  4. 호스트 `Start` → 두 인스턴스 모두 `MultiStageId` 가 가리키는 맵으로 진입.
...
- **Stand-alone 회귀**: `UTDPlayModeSelectWidget` 의 Solo 분기 → `UTDLevelSessionSubsystem::RequestLoadStage` 가 멀티 세션 미경유로 동작하는지 (Play Standalone 호환).
